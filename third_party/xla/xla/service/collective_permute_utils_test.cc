...
HloCollectivePermuteInstruction CreateCollectivePermute(
    const SourceTargetPairs& pairs, HloInstruction* input) {
...
TEST_F(CollectivePermuteUtilsTest, SourceTargetPairsString) {
  std::unique_ptr<HloInstruction> input = HloInstruction::CreateToken();
  EXPECT_EQ(SourceTargetPairsString(
                CreateCollectivePermute(fwd2_.cycle, input.get())),
            "{{0,1},{1,0}}");
  EXPECT_EQ(SourceTargetPairsString(
                CreateCollectivePermute(bwd2_.cycle, input.get())),
            "{{1,0},{0,1}}");
  EXPECT_EQ(SourceTargetPairsString(
                CreateCollectivePermute(fwd4_.cycle, input.get())),
            "{{0,1},{1,2},{2,3},{3,0}}");
  EXPECT_EQ(SourceTargetPairsString(
                CreateCollectivePermute(bwd4_.cycle, input.get())),
            "{{0,3},{1,0},{2,1},{3,2}}");
}
