...
absl::StatusOr<GpuTargetConfig> GetTargetConfigFromFile(
  ...
  TF_RET_CHECK(!debug_opts.xla_gpu_target_config_filename().empty());
...
absl::StatusOr<GpuTopology> InferGpuTopology(
  ...
  if (auto gpu_topology = options.gpu_topology; gpu_topology.has_value()) {
    ...
    // TODO: b/491510579 - This looks wrong, but it's the current behavior.
  ...
  if (!gpu_target_config.has_value() &&
    ...
    ASSIGN_OR_RETURN(gpu_target_config, GetTargetConfigFromFile(debug_opts));
  }
  ...
  if (gpu_target_config.has_value() && stream_exec == nullptr) {
    LOG(INFO) << "Found target compilation environment, and not stream "
                 "executor. Performing deviceless compilation.";
    return GpuTopology{gpu_target_config->device_description.platform_version(),
                       num_partitions, num_hosts_per_partition,
                       num_devices_per_host, std::move(gpu_target_config)};
  }

  if (gpu_target_config.has_value()) {
    LOG(INFO) << "Found target compilation environment, and stream executor. "
  ...
  LOG(INFO)
      << "Found stream executor, and not a target compilation environment. "
  ...
  return GpuTopology{stream_exec->GetDeviceDescription().platform_version(),
                     num_partitions, num_hosts_per_partition,
                     num_devices_per_host, std::move(local_target_config)};
}
