...
LiteRtStatus DynamicUpdateSliceOpLegalization::LegalizeOp(
  ...
  //      output: [a, b, c, d]
  Qnn_OpConfig_t transpose_result_op = BuildDefaultOp();
  std::string transpose_result_op_name = absl::StrFormat(
      kTransposeResultOpFmt, /*increase counter*/ op_counter_++);
  LITERT_RETURN_STATUS_IF_NOT_OK(SetOpInfo(
      transpose_result_op_name.c_str(), kDefaultQnnOpPackageName.data(),
      kTransposeOpTypeName.data(), transpose_result_op));
  ...
  LITERT_RETURN_STATUS_IF_QNN_NOT_OK(BuildAndRegisterQnnOp(
      ...
      &qnn_op_outs[0], transpose_result_op, kTransposeParamSize, &perm_param,
      graph_mapper));
