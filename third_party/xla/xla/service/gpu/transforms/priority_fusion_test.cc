...
ENTRY main {
  param_0 = f32[32,64] parameter(0)
  c_0 = f32[] constant(0)
  ROOT triton_softmax = f32[32] fusion(param_0, c_0), kind=kCustom, calls=triton_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1"]}],"num_warps":"1"}}}
})");
  EXPECT_THAT(priority_fusion_.Run(module.get()), IsOkAndHolds(true));
...
ENTRY main {
  ...
  triton_softmax = f32[125,127]{1,0} fusion(producer_fusion), kind=kCustom, calls=triton_softmax_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","127"]}],"num_warps":"1"}}}
...
ENTRY main {
  param_0 = f32[125]{0} parameter(0)
  producer_fusion = f32[125,127] fusion(param_0), kind=kCustom, calls=producer_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","127"]}],"num_warps":"1"}}}
...
ENTRY main {
  ...
  ROOT triton_fusion = f32[125,127] fusion(producer_fusion, param_1), kind=kCustom, calls=triton_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","127"]}],"num_warps":"1"}}}
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(kHloText));
...
ENTRY main {
  ...
  triton_fusion = f32[125,127] fusion(param_0), kind=kCustom, calls=triton_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","127"]}],"num_warps":"1"}}}
