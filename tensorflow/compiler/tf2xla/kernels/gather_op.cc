 ...
 public:
  ...
  void Compile(XlaOpKernelContext* context) override {
    ...
    if (bad_indices_policy_ == "IGNORE") {
      ...
      OP_REQUIRES(context, gather_shape.ok(),
                  errors::InvalidArgument("Gather shape status is not ok"));

      std::vector<int64_t> valid_mask_dims(gather_shape->dimensions().begin(),
                                           gather_shape->dimensions().end());
