...
absl::Status DeserializeMlirModule(
  ...
  // We don't have a way to know whether the serialized MLIR module is
  // compressed or not. We cannot check the flag
  // tf_serialize_mlir_to_compressed_bytecode here because it adds a dependency
  // cycle.
  // We try to parse as string first. If it fails, we try to uncompress it and
  // then parse it because the default serialization format is textual.
  // Parse module as string.
  if (serialized_mlir_module.starts_with("MLiR")) {
    *mlir_module = mlir::parseSourceString<mlir::ModuleOp>(
        serialized_mlir_module, mlir_context);
    if (!*mlir_module) {
      // Consume the error message from MLIR and log it.
  ...
  auto zlib_stream = std::make_unique<tensorflow::io::ZlibInputStream>(
      input_stream.get(), options.input_buffer_size, options.output_buffer_size,
      options);
  ...
  if (!s.ok() && !absl::IsOutOfRange(s)) {
    ...
    LOG(ERROR) << "Failed to uncompress MLIR module: " << s.message();
  ...
  *mlir_module = mlir::parseSourceString<mlir::ModuleOp>(
  ...
  // Successfully parsed the MLIR module after uncompressing.
  return absl::OkStatus();
}
