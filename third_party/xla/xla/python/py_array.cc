...
void check_sharding_with_device_subset_has_default_layout(
  ...
  absl::flat_hash_set<int> sharding_process_indices;
  for (const auto& device : ifrt_array->sharding().devices()->devices()) {
    sharding_process_indices.insert(device->ProcessIndex());
  }
  bool has_process_with_no_local_shards = false;
  for (const auto& device : ifrt_array->client()->devices()) {
    if (!sharding_process_indices.contains(device->ProcessIndex())) {
      has_process_with_no_local_shards = true;
      break;
    }
  }
  if (!has_process_with_no_local_shards) {
    return;
  }
  auto shard_shape = ifrt_array->sharding().GetShardShape(ifrt_array->shape());
  if (!shard_shape.ok()) {
    throw nb::value_error(
        absl::StrCat("Failed to get shard shape: ", shard_shape.status())
            .c_str());
  }
  ...
  if (absl::IsUnimplemented(default_layout.status())) {
    return;
  }
  if (!default_layout.ok()) {
    throw nb::value_error(
        absl::StrCat("Failed to get default layout: ", default_layout.status())
            .c_str());
  }
  if (!layout.ok()) {
    throw nb::value_error(
        absl::StrCat("Failed to get layout: ", layout.status()).c_str());
  }
...
void PyArray::PyInit(PyArray self, nb::object aval, nb::object sharding,
  ...
  if (skip_checks) {
    ...
    if (!py_arrays.empty()) {
      int sharding_num_devices = nb::cast<int>(sharding.attr("num_devices"));
      auto ifrt_array0 = py_arrays[0].ifrt_array();
      check_sharding_with_device_subset_has_default_layout(
          ifrt_array0, sharding_num_devices);
    }
