...
MeshAttr getSdyMeshAttr(mlir::sdy::TensorShardingAttr sharding,
  ...
  if (auto mesh = mlir::dyn_cast<MeshAttr>(mesh_or_ref)) {
    return mesh;
  }
...
std::optional<xla::OpSharding> ExtractShardyResultShardingFromFrontendAttrs(
  ...
  if (auto custom_call_op =
    ...
    return CreateOpShardingFromSdySharding(
        sharding_per_value_attr.getSharding(0), sdy_meshes, op_frontend_attrs);
  }
  return std::nullopt;
}
