...
// absl::StatusOr<CompileModuleResults> CompileModuleToLlvmIr2(
...
//                       can_share_buffer_function,
//                                           buffer_size_bytes_function));
//   TF_ASSIGN_OR_RETURN(results.output_info,
//                       GetOutputInfo(*hlo_module,
//                       *results.buffer_assignment));
//   DumpHloModuleIfEnabled(*hlo_module, *results.buffer_assignment,
//                          GetDumpName(device_desc));

//   VLOG(1) << "After optimization module fingerprint for " <<
//   hlo_module->name()
//           << ": " << hlo_module->GetFingerprint128();

//   // TF_ASSIGN_OR_RETURN(
//   //     std::unique_ptr<IrEmitterUnnested> ir_emitter,
//   //     LowerHlo(hlo_module, results.buffer_assignment.get(),
//   //              results.execution_stream_assignment.get(),
//   //              results.llvm_module.get(),
//   //              results.llvm_module_constants.get(), platform_name,
//   //              platform_id, device_desc, split_constants_module));
//   std::unique_ptr<IrEmitterUnnested> ir_emitter;
//   results.constants =
//   std::move(ir_emitter->ir_emitter_context().constants()); results.executable
//   = ir_emitter->ConsumeThunkSequence();

//   if (UseCache(hlo_module->config().debug_options(), split_constants_module))
//   {
//     results.kernel_compilation_cache =
//         ir_emitter->ir_emitter_context().kernel_cache().Export();
//   }
