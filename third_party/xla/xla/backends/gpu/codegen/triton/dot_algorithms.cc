...
bool IsTf32Allowed(const PrecisionSpec& precision_spec) {
  if (precision_spec.algorithm == PrecisionConfig::ALG_UNSET) {
    return tsl::tensor_float_32_execution_enabled() &&
           precision_spec.lhs_operand_precision == PrecisionConfig::DEFAULT &&
           precision_spec.rhs_operand_precision == PrecisionConfig::DEFAULT;
  }
  return algorithm_util::HasTf32InputType(precision_spec.algorithm);
}

bool IsF32UpcastFromSubbyteIntegerType(Value v) {
  if (!ElementType(v).isF32()) {
    return false;
  }

  if (auto convert = llvm::dyn_cast<arith::SIToFPOp>(v.getDefiningOp())) {
    return ElementType(convert->getOperand(0)).getIntOrFloatBitWidth() <= 8;
  }

  return false;
}

// TODO(b/320659359): remove dot_operands once the unsupported bitwidth
// precision switch hack is removed.
ttir::InputPrecision InferDotPrecision(const DotOperands& dot_operands,
                                       const PrecisionSpec& precision_spec) {
  if (precision_spec.algorithm == PrecisionConfig::ALG_DOT_TF32_TF32_F32_X3) {
    return ttir::InputPrecision::TF32x3;
  }

  bool use_tf32 = IsTf32Allowed(precision_spec);
  use_tf32 = use_tf32 && !IsF32UpcastFromSubbyteIntegerType(dot_operands.lhs);
  use_tf32 = use_tf32 && !IsF32UpcastFromSubbyteIntegerType(dot_operands.rhs);
...
absl::StatusOr<Value> EmitDotAlgUnset(EmitterLocOpBuilder& b,
  ...
  auto dot_precision = InferDotPrecision(dot_operands, precision_spec);
...
// TODO(bchetioui): remove 'InferDotPrecision' to be more explicit about what
// happens.
absl::StatusOr<Value> EmitRegularDot(EmitterLocOpBuilder& b,
  ...
  return b.create<ttir::DotOp>(
      ...
      /*inputPrecision=*/InferDotPrecision(dot_operands, precision_spec),
      /*maxNumImpreciseAcc=*/max_num_imprecise_acc);
}
...
absl::StatusOr<Value> EmitSingleTileDot(EmitterLocOpBuilder& b,
  ...
  TF_ASSIGN_OR_RETURN(Type expected_output_type,
                      TritonType(b, dot.shape().element_type()));

  if (ElementType(result) != expected_output_type) {
    result = Cast(b, result, expected_output_type);
  }
