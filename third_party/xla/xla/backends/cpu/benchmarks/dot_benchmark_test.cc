...
void BM_Dot(benchmark::State& state, const Shape& shape) {
  absl::string_view hlo_template = R"hlo(
    HloModule benchmark
    ENTRY main {
      p0 = $a_type[$m,$k] parameter(0)
      p1 = $b_type[$k,$n] parameter(1)
      ROOT %result = $c_type[$m,$n] dot(p0, p1), lhs_contracting_dims={1}, rhs_contracting_dims={0}
    }
  )hlo";
        ...
        for (int64_t d1 : {2, 32, 64, 128, 256, 512}) {
          configs.push_back(std::unique_ptr<MultiBenchmarkConfig>(
              RegisterJitAndAotBenchmarks(
                  absl::StrCat("BM_BatchedDot_", in_dtype_str, "_",
                               out_dtype_str, "_", d0, "x", d1, "x", d1),
