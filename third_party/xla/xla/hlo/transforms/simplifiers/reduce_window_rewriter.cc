...
std::vector<int64_t> ReduceWindowRewriter::GetTransposeInputs(
  ...
  if (scan_dim != last_dim) {
    ...
    std::swap(permutation[scan_dim], permutation[last_dim]);
...
absl::StatusOr<bool> ReduceWindowRewriter::TryOptimizeCumSumOrProd(
  ...
  // last. If the scan dimension is not the last dimension, we need to transpose
  // the inputs and then permute the dimensions.
  std::vector<int64_t> permutation =
      GetTransposeInputs(*parent, sources, rank, scan_dim, last_dim);
