...
struct OutputTilingInfo {
  ...
  // TODO(bchetioui): update documentation. Maybe also add active parameters
...
absl::StatusOr<OutputTilingInfo> ComputeOutputTilingInfo(
    ...
    absl::Span<const int64_t> active_tiling_parameters,
    mlir::MLIRContext* mlir_context,
  ...
  for (int64_t dim_id : active_tiling_parameters) {
    ...
    dim_vars[dim_id] = {0, upper_bound - 1};
    dim_vars[dim_id].name = absl::StrCat("pid_", dim_id);
    tiled_dims[dim_id] =
  ...
  IndexingMap output_tile_offset_indexing{
      mlir::AffineMap::get(
          ...
          /*symbolCount=*/0, tiled_dims, mlir_context),
      dim_vars, /*range_vars=*/{}, /*rt_vars=*/{}, constraints};

  // TODO(bchetioui): revisit linearization. This makes it hard to do things
  // like grid tiling, for instance.
  IndexingMap linear_output_tile_offset_indexing =
      LinearizeTileOffsets(output_tile_offset_indexing, outer_loop_bounds,
                           active_tiling_parameters, mlir_context);
...
// An unsafe helper to construct
// `TilingSpecification::InstructionAndNumTilingParameters`.
struct UnsafeInstructionAndNumParameters {
...
absl::Status PopulateNestedParameters(
    ...
    std::vector<UnsafeInstructionAndNumParameters>& parameter_mapping) {
  auto set_mapping_for_instruction =
      [&](const HloInstruction& instruction, int64_t num_parameters) {
        ...
        // example.
        for (UnsafeInstructionAndNumParameters& mapping : parameter_mapping) {
          if (mapping.instruction == &instruction) {
            mapping.num_parameters += num_parameters;
  ...
  const auto& instruction_post_order = fusion.MakeInstructionPostOrder();
  for (auto it = instruction_post_order.rbegin();
       it != instruction_post_order.rend(); ++it) {
    const HloInstructionAdaptor& instruction_adaptor = *it;
...
ParameterMappingFromFusionAdaptor(const HloFusionAdaptor& fusion_adaptor,
  ...
  std::vector<UnsafeInstructionAndNumParameters> unsafe_parameter_mapping;
  ...
  unsafe_parameter_mapping.push_back({&real_root, num_tile_sizes});

  TF_RETURN_IF_ERROR(
      PopulateNestedParameters(fusion_adaptor, unsafe_parameter_mapping));
  TilingSpecification::ParameterMapping parameter_mapping;
  for (const UnsafeInstructionAndNumParameters& unsafe_mapping :
       unsafe_parameter_mapping) {
    TF_ASSIGN_OR_RETURN(
        auto mapping,
        TilingSpecification::InstructionAndNumTilingParameters::Create(
            unsafe_mapping.instruction, unsafe_mapping.num_parameters));
    parameter_mapping.push_back(mapping);
  }
...
absl::StatusOr<int64_t> ParameterIndexImpl(
  ...
  for (const auto& mapping : parameter_mapping) {
    int64_t num_parameters = mapping.num_tiling_parameters();
    if (mapping.instruction() == hlo) {
...
// A type-safe constructor for `InstructionAndNumTilingParameters`. This
// constructor enforces that the number of tiling parameters provided for
// the instruction is either:
//  1. the count of its hidden parameters, or
//  2. the count of its hidden parameters + the count of its output
//     parameters.
/*static*/ absl::StatusOr<
    TilingSpecification::InstructionAndNumTilingParameters>
TilingSpecification::InstructionAndNumTilingParameters::Create(
    const HloInstruction* instruction, int64_t num_tiling_parameters) {
  int64_t num_output_dimensions = instruction->shape().dimensions().size();
  // TODO(bchetioui): handle reductions.
  if (instruction->opcode() == HloOpcode::kDot) {
    int64_t num_contracting_dimensions =
        instruction->dot_dimension_numbers().lhs_contracting_dimensions_size();

    if (num_tiling_parameters != num_contracting_dimensions &&
        num_tiling_parameters !=
            num_contracting_dimensions + num_output_dimensions) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Instruction ", instruction->ToString(), " can only be given ",
          num_contracting_dimensions, " or ",
          num_contracting_dimensions + num_output_dimensions,
          " tiling parameters but was given ", num_tiling_parameters, "."));
    }
  } else if (num_tiling_parameters != num_output_dimensions) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Instruction ", instruction->ToString(), " can only be given ",
        num_output_dimensions, " tiling parameters but was given ",
        num_tiling_parameters, "."));
  }

  return InstructionAndNumTilingParameters(instruction, num_tiling_parameters);
}
...
absl::StatusOr<std::vector<int64_t>> Tiling::Flatten(
  ...
  std::vector<int64_t> linearized_tile_sizes;
  linearized_tile_sizes.reserve(tiling_specification.num_parameters());
  for (const auto& mapping : tiling_specification.parameter_mapping()) {
    ...
    if (tile_sizes.size() != mapping.num_tiling_parameters()) {
      return absl::FailedPreconditionError(absl::StrCat(
          "Instruction ", mapping.instruction()->ToString(),
          " was expected to have ", mapping.num_tiling_parameters(),
          " tile sizes but had ", tile_sizes.size(), "."));
    }
    linearized_tile_sizes.insert(linearized_tile_sizes.end(),
                                 tile_sizes.begin(), tile_sizes.end());
  }

  return linearized_tile_sizes;
}
...
bool Tiling::ConformsTo(const TilingSpecification& tiling_specification) const {
  ...
  absl::StatusOr<std::vector<int64_t>> linearized_tile_sizes_or =
      Flatten(tiling_specification);
  if (!linearized_tile_sizes_or.ok()) {
    return false;
  }

  return tiling_specification.constraints().IsSatisfiedBy(
      *linearized_tile_sizes_or);
}
...
std::vector<int64_t> InputSpaceForParameterMapping(
  ...
  // TODO(bchetioui): store total number of params?

  for (const auto& mapping : parameter_mapping) {
    const HloInstruction* hlo = mapping.instruction();
    int64_t num_parameters = mapping.num_tiling_parameters();
    // TODO(bchetioui): handle reductions.
  ...
  for (const auto& mapping : parameter_mapping) {
    ...
    if (hlo == root) {
      int64_t num_hidden_parameters =
          mapping.num_tiling_parameters() - num_output_parameters;
      for (int64_t parameter_index = num_hidden_parameters;
           parameter_index < mapping.num_tiling_parameters();
           ++parameter_index) {
    ...
    dim_offset += mapping.num_tiling_parameters();
  }
...
// TODO(bchetioui): add documentation of expectations.
IndexingMap InsertTilingParameterForContractingDimensions(
  ...
  // TODO(b/...): handle reductions here as well once priority fusion can handle
  // it. This discrepancy will prevent us from reducing.
  // TODO(bchetioui): by adding a special path for reductions, we can handle
  // them here as well, even without nests.
...
/*static*/ SymbolicTileAnalysisOrError SymbolicTileAnalysis::AnalyzeFusionImpl(
  ...
  while (!worklist.empty()) {
    ...
    for (auto [operand_index, operand_and_indexing_map_set] :
      ...
      // TODO(bchetioui): I'm sure this'll behave badly if there's a contracting
      // dimension of size 1.
      ...
      if (operand_indexing_map.GetRangeVarsCount() != 0) {
        operand_indexing_map = InsertTilingParameterForContractingDimensions(
            hlo, operand_index, parameter_mapping, operand_indexing_map);
      }
      ...
      if (operand.opcode() == HloOpcode::kFusion &&
        ...
        auto analysis_or = SymbolicTileAnalysis::AnalyzeNestedFusion(
            *nested_fusion_adaptor, parameter_mapping, ctx,
            operand_indexing_map, simplification_mode,
  ...
  // TODO(bchetioui): migrate emitter-specific constraints to using the new
  // `TilingSpecification`.
...
/*static*/ SymbolicTileAnalysisOrError SymbolicTileAnalysis::AnalyzeFusion(
  ...
  if (!real_root_index_or.ok()) {
    return FusionDecision::Forbid(real_root_index_or.status().message());
  }

  auto parameter_mapping_or =
      ParameterMappingFromFusionAdaptor(fusion, *real_root_index_or);
  if (!parameter_mapping_or.ok()) {
    return FusionDecision::Forbid(parameter_mapping_or.status().message());
  }
...
absl::StatusOr<bool> SymbolicTileAnalysis::ParametersSatisfyConstraints(
  ...
  ::xla::gpu::Tiling::TileMapping tile_mapping(
      {{real_root, absl::InlinedVector<int64_t, 4>(tile_parameters.begin(),
                                                   tile_parameters.end())}});
  ::xla::gpu::Tiling tiling(tile_mapping);
...
absl::StatusOr<bool> IsSafeForBufferSharing(const TiledHloInstruction& output,
  ...
  std::vector<int64_t> active_tiling_parameters;
  active_tiling_parameters.reserve(output.hlo()->shape().dimensions().size());
  for (int64_t i = 0; i < output.hlo()->shape().dimensions().size(); ++i) {
    active_tiling_parameters.push_back(i);
  }
  // TODO(bchetioui): fix this derivation.
  TF_ASSIGN_OR_RETURN(
      auto tiling_info,
...
absl::StatusOr<TiledHloComputation> ComputeTiledHloInstructionsImpl(
  ...
  for (mlir::AffineExpr expr : real_root_indexing.GetAffineMap().getResults()) {
    for (int64_t dim_id : ExtractDimensionIds(expr)) {
      if (absl::c_find(active_tiling_parameters, dim_id) ==
          active_tiling_parameters.end()) {
        active_tiling_parameters.push_back(dim_id);
      }
    }
  }
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      ComputeOutputTilingInfo(real_root_indexing, flat_tiling_parameters,
                              active_tiling_parameters, context,
                              parent_output_tile_dim_bounds));
    ...
    if (const auto* symbolic_fusion_tiling =
      ...
      TF_ASSIGN_OR_RETURN(
          ...
          ComputeTiledHloInstructionsImpl(
              ...
              active_tiling_parameters, compute_all_tile_offset_indexing_maps,
...
SymbolicTileAnalysis::ComputeTiledHloInstructions(
  ...
  if (!constraints_are_known_satisfied) {
    ...
    if (!parameters_satisfy_constraints) {
      // TODO(bchetioui): improve this error message.
      return absl::InvalidArgumentError(absl::StrCat(
          "Tiling ", "TODO: print tiling ", "does not satisfy constraints."));
    }
  }
  ...
  return ComputeTiledHloInstructionsImpl(
      *this, flat_tiling_parameters, /*active_tiling_parameters=*/{},
