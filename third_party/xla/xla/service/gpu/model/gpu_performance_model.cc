...
GpuPerformanceModel::EstimateRunTimeForInstruction(
  ...
  GpuPerformanceModelCache gpu_analysis_cache;
  GpuPerformanceModel model{device_info, fusion_analysis_cache,
                            gpu_analysis_cache};
  return model.EstimateRunTimeForInstruction(instr, cost_analysis);
}

EstimateRunTimeData GpuPerformanceModel::EstimateRunTimeForInstructionCached(
    const HloInstruction* instr, const GpuHloCostAnalysis* cost_analysis) {
  if (auto cached_result_opt = gpu_performance_model_cache_.Get(*instr)) {
    return *cached_result_opt;
  }

  auto runtime_data = EstimateRunTimeForInstruction(instr, cost_analysis);
...
/*static*/ GpuPerformanceModel::RunTimes
GpuPerformanceModel::EstimateRunTimesForMultiOutputFusion(
    const HloInstruction* producer, const HloInstruction* consumer,
    const se::DeviceDescription& device_info,
    const GpuHloCostAnalysis* cost_analysis) {
  HloFusionAnalysisCache fusion_analysis_cache{device_info};
  GpuPerformanceModelCache gpu_performance_model_cache;
  return GpuPerformanceModel{device_info, fusion_analysis_cache,
                             gpu_performance_model_cache}
      .EstimateRunTimesForMultiOutputFusion(producer, consumer, cost_analysis);
}
