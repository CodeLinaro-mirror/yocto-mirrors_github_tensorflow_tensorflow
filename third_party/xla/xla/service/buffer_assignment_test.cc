...
TEST(BufferAllocationSliceProtoTest, FromProtoErrorAllocationNotFound) {
  ...
  EXPECT_EQ(status_or_slice.status().code(), absl::StatusCode::kOutOfRange);
  EXPECT_THAT(
      status_or_slice.status().message(),
      ::testing::HasSubstr("Buffer allocation index 2 is out of range."));
}
