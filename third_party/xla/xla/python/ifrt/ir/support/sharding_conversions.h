...
//
// If `device_ids` is provided, then a HloShardingV1 is returned. Otherwise,
// IOTA is assumed and a HloShardingV2 is returned.
//
// The function assumes that `sharding_param` is valid.
absl::StatusOr<xla::HloSharding> ToHloSharding(
    const ShardingParam& sharding_param,
    std::optional<llvm::ArrayRef<int>> device_ids = std::nullopt);
...
struct ShardingParamWithDeviceIds {
  ...
  // If `device_ids` is nullopt, then the device ids are assumed to be IOTA.
