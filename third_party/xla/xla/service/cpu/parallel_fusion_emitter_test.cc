...
TEST_F(ParallelFusionEmitterTest, FusionsAreSorted) {
  ...
  const HloFusionInstruction* fusion_0 =
      Cast<HloFusionInstruction>(root_tuple->operand(0));
  const HloFusionInstruction* fusion_1 =
      Cast<HloFusionInstruction>(root_tuple->operand(1));

  BlockingThreadPool blocking_thread_pool;
  tsl::thread::ThreadPool thread_pool(&blocking_thread_pool);

  xla::cpu::ParallelFusionEmitter fussion_emitter(
      thread_pool, CreateDefaultOptions(), CreateMockHooks(2), nullptr, false);
