...
int main(int argc, char** argv) {
  ...
  if (!tsl::io::GetTestUndeclaredOutputsDir(
          &compile_options->dot_graph_dump_to)) {
#if defined(PLATFORM_GOOGLE)
    LOG(WARNING) << "Not running a test. `dot_graph_dump_to` compile "
                    "optionmust be specified to dump the dot graph";
    compile_options->dot_graph_dump_to = "";
#else
    compile_options->dot_graph_dump_to = tsl::testing::TmpDir();
#endif
  }
