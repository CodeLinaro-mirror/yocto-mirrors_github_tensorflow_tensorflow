...
RaggedAllToAllStartThunk::InitializeOnce(const InitializeParams& params) {
  ...
  if ((is_local(params.local_device_count) &&
       use_multi_gpu_barrier_in_one_shot_kernel_) ||
      (is_local_to_fast_interconnect(params.local_device_count) &&
       use_multi_gpu_barrier_with_nccl_in_one_shot_kernel_)) {
...
absl::Status RaggedAllToAllStartThunk::Initialize(
  ...
  if (is_local_to_fast_interconnect(params.local_device_count) &&
      use_multi_gpu_barrier_with_nccl_in_one_shot_kernel_) {
...
absl::Status RaggedAllToAllStartThunk::RunCollective(
  ...
  if (is_local_to_fast_interconnect(
          params.collective_params->local_device_count) &&
      use_multi_gpu_barrier_with_nccl_in_one_shot_kernel_) {
