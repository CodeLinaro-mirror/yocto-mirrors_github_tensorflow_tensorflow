...
std::unique_ptr<CustomCallThunk> CreateCustomCallThunk(
  ...
  auto thunk = CustomCallThunk::Create(
      Thunk::ThunkInfo(), call_target, CustomCallThunk::CustomCallTarget(),
      /*operands=*/std::vector<std::optional<CustomCallThunk::Slice>>{},
      /*results=*/std::vector<std::optional<CustomCallThunk::Slice>>{},
      /*opaque=*/"");
...
TEST(CommandBufferConversionPassTest,
  ...
  EXPECT_EQ(thunks_in_command_buffer[0]->kind(), Thunk::kCustomCall);
}
}  // namespace gpu
}  // namespace xla
