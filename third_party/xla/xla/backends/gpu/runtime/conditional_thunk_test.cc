...
TEST(ConditionalThunkTest, BufferUses) {
  ...
  bool branch_index_is_bool = false;
  ConditionalThunk conditional_thunk(Thunk::ThunkInfo(), branch_index_slice,
                                     std::move(branch_thunks),
                                     branch_index_is_bool);

  EXPECT_EQ(conditional_thunk.branch_count(), 1);
  EXPECT_EQ(conditional_thunk.branch_index_is_bool(), branch_index_is_bool);
  EXPECT_EQ(conditional_thunk.branch_index_buffer(), branch_index_slice);
  EXPECT_EQ(conditional_thunk.branch_thunks().size(), 1);
  EXPECT_EQ(conditional_thunk.branch_thunks()[0]->thunks().size(), 1);
  EXPECT_EQ(conditional_thunk.branch_thunks()[0]->thunks()[0]->kind(),
            Thunk::Kind::kGemm);
}
}  // namespace
}  // namespace xla::gpu
