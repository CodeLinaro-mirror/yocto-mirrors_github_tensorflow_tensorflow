...
TEST(WhileThunkTest, NonBlockingExecute) {
  ...
  auto pred = LiteralUtil::CreateR0<uint8_t>(false);
  auto cnt = LiteralUtil::CreateR0<int32_t>(0);

  BufferAllocations allocations = CreateBufferAllocations(pred, cnt);

  auto [pred_alloc, cnt_alloc] = CreateBufferAllocation(pred, cnt);
  auto [pred_slice, cnt_slice] =
      CreateBufferAllocationSlice(pred_alloc, cnt_alloc);
  ...
  EXPECT_EQ(cnt, LiteralUtil::CreateR0<int32_t>(kNumIterations));
}
