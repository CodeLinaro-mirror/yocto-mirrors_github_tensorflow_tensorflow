...
struct XnnFusionThunk::XnnRuntime {
  ...
  tsl::AsyncValueRef<XnnFusionThunk::ExecuteEvent> Invoke(
      ...
      absl::FunctionRef<bool(size_t)> is_value_argument,
      absl::FunctionRef<bool(size_t)> is_value_result);
...
XnnFusionThunk::XnnRuntime::Invoke(
  ...
  for (const se::DeviceMemoryBase& argument : arguments) {
    ...
    if (!is_value_argument(value.id)) external_values.push_back(value);
  }

  for (const se::DeviceMemoryBase& result : results) {
    xnn_external_value value{id++, result.opaque()};
    if (!is_value_result(value.id)) external_values.push_back(value);
  }
...
XnnFusionThunk::XnnFusionThunk(Options options, Info info,
    ...
    : Thunk(Kind::kXnnFusion, std::move(info)),
      ...
      value_arguments_(value_arguments.begin(), value_arguments.end()),
      value_results_(value_results.begin(), value_results.end()),
...
tsl::AsyncValueRef<XnnFusionThunk::ExecuteEvent> XnnFusionThunk::Execute(
  ...
  auto invoke = [&](XnnRuntime& runtime) {
    tsl::AsyncValueRef<ExecuteEvent> executed = runtime.Invoke(
        ...
        [&](size_t id) { return value_arguments_.contains(id); },
        [&](size_t id) { return value_results_.contains(id); });
