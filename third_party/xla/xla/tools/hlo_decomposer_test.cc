...
ENTRY main {
  ...
  HloInstruction* subtract =
      module->entry_computation()->GetInstructionWithName("subtract");
  auto new_module =
      ExtractProducerConsumersIntoNewModule(*dot, {add, subtract});
...
ENTRY main {
  ...
  HloInstruction* add =
      module->entry_computation()->GetInstructionWithName("add");
  auto new_module = ExtractProducerConsumersIntoNewModule(*dot, {add});
