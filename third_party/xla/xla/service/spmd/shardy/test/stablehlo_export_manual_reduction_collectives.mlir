...
func.func @sharded_to_unreduced(%arg0: tensor<8x8xf32> {sdy.sharding = #sdy.sharding<@mesh, [{"x"}, {"y"}]>}) -> tensor<8x8xf32> {
  // CHECK-NEXT: %[[MANUAL_COMP:.*]] = sdy.manual_computation(%arg0)
  ...
  // CHECK-NEXT:   %[[BROADCAST:.*]] = stablehlo.broadcast %[[CST]], sizes = [8, 4] : (tensor<f32>) -> tensor<8x4xf32>
  ...
  // CHECK-NEXT:   %[[REM1:.*]] = stablehlo.remainder %[[PID_I32]], %[[C2]] : tensor<i32>
  // CHECK-NEXT:   %[[DIV1:.*]] = stablehlo.divide %[[PID_I32]], %[[C2]] : tensor<i32>
  // CHECK-NEXT:   %[[C4:.*]] = stablehlo.constant dense<4> : tensor<i32>
  // CHECK-NEXT:   %[[REM2:.*]] = stablehlo.remainder %[[DIV1]], %[[C4]] : tensor<i32>
  // CHECK-NEXT:   %[[DIV2:.*]] = stablehlo.divide %[[DIV1]], %[[C4]] : tensor<i32>
  // CHECK-NEXT:   %[[C0:.*]] = stablehlo.constant dense<0> : tensor<i32>
  // CHECK-NEXT:   %[[DIM0:.*]] = stablehlo.get_dimension_size %[[ARG1]], dim = 0 : (tensor<2x4xf32>) -> tensor<i32>
  // CHECK-NEXT:   %[[OFFSET:.*]] = stablehlo.multiply %[[REM2]], %[[DIM0]] : tensor<i32>
  // CHECK-NEXT:   %[[ADD:.*]] = stablehlo.add %[[C0]], %[[OFFSET]] : tensor<i32>
  // CHECK-NEXT:   %[[C4_2:.*]] = stablehlo.constant dense<4> : tensor<i32>
  // CHECK-NEXT:   %[[STRIDE:.*]] = stablehlo.multiply %[[DIM0]], %[[C4_2]] : tensor<i32>
  // CHECK-NEXT:   %[[C0_2:.*]] = stablehlo.constant dense<0> : tensor<i32>
  // CHECK-NEXT:   %[[DIM1:.*]] = stablehlo.get_dimension_size %[[ARG1]], dim = 1 : (tensor<2x4xf32>) -> tensor<i32>
  // CHECK-NEXT:   %[[DUS:.*]] = stablehlo.dynamic_update_slice %[[BROADCAST]], %[[ARG1]], %[[ADD]], %[[C0_2]] : (tensor<8x4xf32>, tensor<2x4xf32>, tensor<i32>, tensor<i32>) -> tensor<8x4xf32>
  // CHECK-NEXT:   sdy.return %[[DUS]] : tensor<8x4xf32>
  // CHECK-NEXT: } : (tensor<8x8xf32>) -> tensor<8x8xf32>
  // CHECK-NEXT: return %[[MANUAL_COMP]] : tensor<8x8xf32>
  %0 = sdy.sharded_to_unreduced [{"x"}, {}] %arg0 out_sharding=<@mesh, [{}, {"y"}], unreduced={"x"}> : tensor<8x8xf32>
  return %0 : tensor<8x8xf32>
}
