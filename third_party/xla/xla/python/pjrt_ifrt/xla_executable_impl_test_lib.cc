...
absl::StatusOr<const LoadedExecutableRef> SimpleAddExecutable(bool serialize) {
  TF_ASSIGN_OR_RETURN(auto client, test_util::GetClient());
  static constexpr absl::string_view kModule = R"(
module @add attributes {
  ...
  Compiler* compiler = client->GetDefaultCompiler();
  return CompileOnDevices(client.get(), compiler, kModule,
...
TEST_P(LoadedExecutableImplTest, GetHloModules) {
  TF_ASSERT_OK_AND_ASSIGN(const LoadedExecutableRef executable,
                          SimpleAddExecutable(/*serialize=*/GetParam()));
