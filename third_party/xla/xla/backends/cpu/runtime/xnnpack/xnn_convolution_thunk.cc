...
XnnConvolutionThunk::Create(
  ...
  if (dnums.kernel_input_feature_dimension() != 3 ||
    ...
    return InvalidArgument(
        "XNNPACK convolution expectes kernel (filter) in OHWI format");
  }
  ...
  return absl::WrapUnique(
      new XnnConvolutionThunk(std::move(options), std::move(info),
                              std::move(slices), std::move(canonical_dims)));
}
...
XnnConvolutionThunk::XnnConvolutionThunk(
    ...
    : XnnFusionThunk(std::move(options), std::move(info),
                     ...
                     /*value_arguments=*/{1}, /*value_results=*/{}),
