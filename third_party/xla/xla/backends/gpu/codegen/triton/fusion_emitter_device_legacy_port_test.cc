...
TEST_F(TritonGemmTest, DISABLED_TODOFP8DotSmallTileDoesNotCrash) {
...
TEST_F(TritonTest, DISABLED_TODOCodegenDynamicSliceWithCorrectOffsets) {
...
TEST_F(TritonGemmTest, DISABLED_TODOMultipleDims) {
  constexpr absl::string_view kHloText = R"(
HloModule t
...
TEST_F(TritonGemmTest, DISABLED_TODOS8xS8) {
  constexpr absl::string_view kHloText = R"(
HloModule t
...
TEST_F(TritonGemmTest, DISABLED_TODOSplitLhsNoncontractingTransposeRhs) {
  constexpr absl::string_view kHloText = R"(
HloModule t
...
TEST_F(TritonGemmTest, DISABLED_TODOSplitLhsNoncontracting) {
  constexpr absl::string_view kHloText = R"(
ENTRY e {
...
// TODO(b/393299275): this should be consolidated into an extensive convert test
// in fusion_emitter_deviceless_test.cc, since it doesn't need to have a dot,
// nor does it need a device.
// TODO(b/393299275): this test requires mixed-type dots to be enabled.
TEST_F(TritonTest, DISABLED_TODOFloatToSignedIntConversion) {
  constexpr absl::string_view kHloText = R"(
triton_gemm_r {
  p0 = s8[32,32]{1,0} parameter(0)
  p1 = f16[32,32]{1,0} parameter(1)
  convert = s8[32,32]{1,0} convert(p1)
  ROOT dot = f32[32,32]{1,0} dot(p0, convert),
    lhs_contracting_dims={1}, rhs_contracting_dims={1}
}
...
ENTRY e {
  p0 = s8[32,32]{1,0} parameter(0)
  p1 = f16[32,32]{1,0} parameter(1)
  ROOT triton_gemm_r = f32[32,32]{1,0} fusion(p0, p1), kind=kCustom,
    calls=triton_gemm_r,
    backend_config={"fusion_backend_config": {kind: "__triton_gemm",
...
TEST_F(TritonGemmTest, DISABLED_TODOFailForTooComplexTiling) {
  auto cc = se::CudaComputeCapability::Ampere();
  const se::DeviceDescription device_info =
      TestGpuDeviceInfo::RTXA6000DeviceInfo();
  llvm::LLVMContext llvm_ctx;
...
ENTRY entry {
  ...
  EXPECT_OK(TritonWrapper("test_fn", fusion2, cc, device_info,
...
TEST_F(
    TritonGemmTest,
    DISABLED_TODOBroadcastsOfTriviallySizedContractingDimensionsAreSupported) {
  constexpr absl::string_view kHloText = R"(
f {
  a = f16[2] parameter(0)
...
// TODO(b/393299275): this requires adding support for dynamic-slice in the
// generic Triton emitter.
TEST_F(TritonGemmTest, DISABLED_TODODynamicSliceIsSupportedInLhsEndToEnd) {
...
// TODO(b/393299275): this requires adding support for dynamic-slice in the
// generic Triton emitter.
TEST_F(TritonGemmTest, DISABLED_TODODynamicSliceIsSupportedInRhs) {
  // The start index(es) for the non-majormost dimension(s) are constant zero(s)
  // because we don't support dynamic slice on those dimensions.
  constexpr absl::string_view kHloText = R"(
HloModule m
...
TEST_P(TritonGemmDynamicSliceClampingTest,
       DISABLED_TODODynamicSliceIsSupportedWhenTheStartIndexNeedsClamping) {
  // The start index(es) for the non-majormost dimension(s) are constant zero(s)
  // because we don't support dynamic slice on those dimensions.
...
// generic Triton emitter.
TEST_F(TritonGemmTest,
       DISABLED_TODODynamicSliceOfMajormostContractingDimIsSupported) {
...
// generic Triton emitter.
TEST_F(TritonGemmTest,
       DISABLED_TODODynamicSliceOfMajormostBatchDimIsSupported) {
  // Tests that dynamic-slice works on the majormost dimension even if that
...
// generic Triton emitter.
TEST_F(TritonGemmTest,
       DISABLED_TODODynamicSliceSingleDimensionIntoReshapeIsSupported) {
...
TEST_F(TritonGemmTestWithSplitK,
       DISABLED_TODOSplitKDoesNotBreakSlicedFragmentedContractingDimension) {
  constexpr absl::string_view kHloText = R"(
ENTRY e {
...
TEST_F(TritonGemmTest, DISABLED_TODOParameterAfterDotIsFused) {
  if (!SupportsBF16(GpuComputeCapability())) {
    GTEST_SKIP() << "BF16 not supported.";
  }
  constexpr absl::string_view kHloText = R"(
...
// and the underlying indexing map is incorrect. This requires a fix in
// symbolic tile derivation.
TEST_F(TritonGemmTest, DISABLED_TODOSplitLHSInputOutputIsFused) {
  if (!SupportsBF16(GpuComputeCapability())) {
    GTEST_SKIP() << "BF16 not supported.";
  }
  if (std::holds_alternative<se::RocmComputeCapability>(
...
TEST_F(CompareTest, DISABLED_TODOSplitK) {
  if (!SupportsBF16(GpuComputeCapability())) {
    GTEST_SKIP() << "BF16 not supported.";
  }
...
TEST_F(CompareTest, DISABLED_TODOSupportsSplitKWithIndivisibleKComplexExample) {
  constexpr absl::string_view kHloTextRef = R"(
dot {
...
// TODO(b/393299275): transform this test once padding derivation if fixed.
TEST_F(CompareTest,
       DISABLED_TODOSupportsSplitKWithIndivisibleKUsingPaddingEqual1) {
  constexpr absl::string_view kHloTextRef = R"(
...
TEST_F(CompareTest,
       DISABLED_TODOSupportsSplitKWithIndivisibleKUsingPaddingEqual5) {
  constexpr absl::string_view kHloTextRef = R"(
HloModule extracted
...
// and the underlying indexing map is incorrect. This requires a fix in
// symbolic tile derivation.
TEST_F(CompareTest, DISABLED_TODODifferentLayoutsAreSupportedInOneScope) {
  const std::string kHloTextTest = R"(
triton_dot {
...
TEST_F(TritonTest, DISABLED_TODOBF16ToFP8EndToEnd) {
  if (!GetCudaComputeCapability().IsAtLeastHopper()) {
    GTEST_SKIP() << "Doesn't pass on pre-Hopper GPUs.";
  }
...
TEST_F(TritonTest, DISABLED_TODOFP8ToFP8EndToEnd) {
  if (!GetCudaComputeCapability().IsAtLeastHopper()) {
    GTEST_SKIP() << "Doesn't pass on pre-Hopper GPUs.";
  }
