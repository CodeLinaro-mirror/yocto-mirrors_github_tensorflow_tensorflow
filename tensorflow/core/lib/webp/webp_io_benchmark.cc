...
static void BM_DecodeLargeImage(benchmark::State& state) {
  ...
  const std::string large_lossy =
      GenerateSyntheticWebP(w_target, h_target, false);
  const std::string large_lossless =
      GenerateSyntheticWebP(w_target, h_target, true);

  const std::string* webp_data = lossless ? &large_lossless : &large_lossy;
