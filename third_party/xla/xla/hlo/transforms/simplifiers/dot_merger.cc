...
absl::StatusOr<HloInstruction*> TryMergeDiffOperand(HloInstruction* a,
                                                    HloInstruction* b) {
...
absl::StatusOr<HloInstruction*> TryMergeOperand(HloInstruction* a,
  ...
  merged = TryMergeDiffOperand(a, b);
  if (!merged.ok() || merged.value() != nullptr) {
    return merged;
  }
  return TryMergeDiffOperand(b, a);
}
...
absl::StatusOr<bool> MergeDots(HloComputation* comp, int64_t max_size_to_merge,
  ...
  for (HloInstruction* instr : comp->instructions()) {
    ...
    for (HloInstruction* operand : instr->operands()) {
      ...
      // DotDecomposer inserts transposes to establish a normal form. Tranposed
