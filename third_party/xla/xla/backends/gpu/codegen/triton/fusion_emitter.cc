...
absl::StatusOr<int64_t> GetDotLoopIterationCount(
  ...
  QCHECK(tiled_dot.hlo()->opcode() == HloOpcode::kDot);
  ...
  if (dims.lhs_contracting_dimensions_size() != 1) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Expected dot to have a singled contracting dimension, got ",
  ...
  if (fusion_tile_sizes.size() <= contracting_dim_idx) {
    return absl::InvalidArgumentError(
        absl::StrCat("Number of tile sizes of the nested fusion root is ",
                     fusion_tile_sizes.size(),
                     " <= contracting dimension index ", contracting_dim_idx));
  }
  int64_t tile_k = fusion_tile_sizes[contracting_dim_idx];
  QCHECK_GT(tile_k, 0);
  int64_t iterations_count = k / tile_k;
  VLOG(2) << absl::StrCat("dot loop_iteration_count ", iterations_count, " = ",
                          k, " / ", tile_k);
  if (iterations_count * tile_k != k) {
    return absl::InternalError(absl::StrCat("Contracting dimension (", k,
                                            ") is not divisible by tile size (",
                                            tile_k, ")"));
  }
...
absl::StatusOr<ScalarOrTensor> EmitDot(
    ...
    const se::DeviceDescription& device_info,
    const HloFusionInstruction* fusion,
    const TiledHloInstruction& tiled_hlo_dot, mlir::triton::FuncOp fn,
    ValueRange tile_multi_index,
    absl::flat_hash_map<const TiledHloInstruction*, ScalarOrTensor>& values) {
  ...
  Where fusions `left` and `right` are tiled for the loop. They also get the
  parameters `kernel` directly, not the results of other instructions. From that
  we will emit:
  ...
  */
  ...
  {  // Loop body.
    ...
    std::vector<TensorValue> dot_args;
    ...
    mlir::Value ki = for_op.getInductionVar();
    ...
    b.create<mlir::scf::YieldOp>(SmallVector<Value>{dot_result});
  }
...
absl::StatusOr<ScalarOrTensor> EmitTiledHloInstruction(
  ...
  if (fusion->IsUserOf(hlo)) {
    // If fusion instruction is a user of hlo instruction then hlo are passed as
    // a parameter. HLO instruction itself might not necessarily be a kParameter
    // but the instruction that produced the operand of the fusion. We need to
    // emit a load from it.
    int64_t arg_index = fusion->operand_index(hlo);

    bool is_nested_fusion = fusion->parent()->IsFusionComputation();
    // Nested fusion is a special case: the parameter it receives MUST be a
    // parameter (kParameter) of the real fusion. And we load it in the context
    // of the real fusion.
    if (is_nested_fusion) {
      QCHECK(hlo->opcode() == HloOpcode::kParameter);
      arg_index = hlo->parameter_number();
    }
...
absl::StatusOr<std::vector<ScalarOrTensor>> EmitTiledComputation(
  ...
  for (const TiledHloInstruction* tiled_hlo :
    ...
    if (tiled_hlo->hlo()->opcode() == HloOpcode::kFusion) {
      // Skip the fusion, we will emit it along with the (dot) user.
      ...
      VLOG(1) << "Skipping nested fusion: " << tiled_hlo->hlo()->ToString();
      continue;
    }
    TF_ASSIGN_OR_RETURN(
        ScalarOrTensor result,
