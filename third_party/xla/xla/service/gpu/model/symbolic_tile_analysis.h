...
class TilingSpecification {
 public:
  ...
  class InstructionAndNumTilingParameters {
   public:
    ...
    //     parameters.
    static absl::StatusOr<InstructionAndNumTilingParameters> Create(
    ...
    // The number of tile sizes that must be specified for `instruction()`.
    int64_t num_tiling_parameters() const { return num_tiling_parameters_; }

   private:
    explicit InstructionAndNumTilingParameters(
        const HloInstruction* instruction, int64_t num_tiling_parameters)
        : instruction_(instruction),
          num_tiling_parameters_(num_tiling_parameters) {}

    const HloInstruction* instruction_;
    int64_t num_tiling_parameters_;
  };
 ...
 private:
  ...
  explicit TilingSpecification(ParameterMapping parameter_mapping,
    ...
    num_parameters_ = 0;
    for (const auto& instruction_and_num_tiling_parameters :
         parameter_mapping_) {
      num_parameters_ +=
          instruction_and_num_tiling_parameters.num_tiling_parameters();
    }
  };
...
class Tiling {
 public:
  ...
  // Returns a flattened list of tile sizes TODO
...
class EmitterSpecificConstraints {
 public:
  ...
  //
  // The tiling parameters are expected to be linearized with regards to the
  // `SymbolicTileAnalysis`'s underlying `TilingSpecification`.
...
class SymbolicTileAnalysis {
 public:
  ...
  // TODO(bchetioui): replace uses with calls to
  ...
  // Returns the tiling specification for the fusion that was used to construct
  // the analysis.
  //
  // The tiling specification wraps the shape of the tiling parameters, and
  ...
  //
  // TODO(bchetioui): deprecate `SymbolicTileAnalysi::Tiling` everywhere to
  ...
  // Returns `true` if a `TileMapping` conforms to the symbolic tile analysis's
  // tiling specification. If provided, also checks the emitter-specific
  // constraints.
  //
  // Returns `false` if the mapping does not conform to the tiling
  // specification.
