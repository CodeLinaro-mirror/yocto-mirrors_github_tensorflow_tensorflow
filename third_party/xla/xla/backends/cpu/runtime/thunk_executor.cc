...
ThunkExecutor::CreateNodeDefs(std::vector<NodeDefBuilder> builders) {
  ...
  defs.reserve(builders.size());

  size_t in_edges_offset = 0;
  size_t out_edges_offset = 0;
  ...
  for (const NodeDefBuilder& b : builders) {
    ...
    defs.push_back(NodeDef{
        b.id,
        b.priority,
        absl::MakeConstSpan(&*inserted_in_edges, b.in_edges.size()),
        absl::MakeConstSpan(&*inserted_out_edges, b.out_edges.size()),
    });

    in_edges_offset += b.in_edges.size();
    out_edges_offset += b.out_edges.size();
  }

  DCHECK_EQ(in_edges_offset, num_in_edges);
  DCHECK_EQ(out_edges_offset, num_out_edges);
