...
static absl::Status SendPtrToPeer(void* ptr, RankId peer, Communicator* comm,
  ...
  VLOG(3) << absl::StreamFormat(
      "SendPtrToPeer on device #%d; peer=%d; comm=%p; stream=%p",
...
absl::Status RunMemCpyRaggedAllToAll(
  ...
  *send_pointer =
  ...
  for (int peer = 0; peer < num_ranks; ++peer) {
    TF_RETURN_IF_ERROR(SendPtrToPeer(send_pointer, RankId(peer), comm, stream));
    TF_RETURN_IF_ERROR(RecvPtrFromPeer(&receive_pointer_map[peer], RankId(peer),
                                       comm, stream));
  }
  TF_RETURN_IF_ERROR(collectives->GroupEnd());
