...
TEST_F(HloTraversalTest, AdaptorUsers) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
    computation1 {
      ...
      add = f32[] add(p0.1, neg.1)
      ROOT tuple = (f32[], f32[]) tuple(neg.1, add)
    }
    ...
    ENTRY entry {
      ...
      add = f32[] add(p0.3, gte)
      fusion2 = f32[] fusion(gte), kind=kLoop, calls=computation2
      exp = f32[] exponential(fusion2)
      ROOT tuple = (f32[], (f32[], f32[]), f32[], f32[]) tuple(add, fusion1, fusion2, exp)
    }
  )"));
  ...
  EXPECT_THAT(add.GetUsers(), ElementsAre(InstructionAdaptorName("mul"),
                                          InstructionAdaptorName("tuple.1")));
