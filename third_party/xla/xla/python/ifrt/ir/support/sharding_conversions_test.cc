...
TEST_P(HloShardingV1RoundtripTest, Roundtrip) {
  ...
  if (result.device_ids.has_value()) {
    device_ids.emplace(*result.device_ids);
  }
  TF_ASSERT_OK_AND_ASSIGN(const HloSharding roundtrip_sharding,
                          ToHloSharding(result.sharding_param, device_ids));
