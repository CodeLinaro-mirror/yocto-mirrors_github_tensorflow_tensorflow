...
__device__ __forceinline__ void ValidateCudaArch() {
...
__device__ __forceinline__ void AtomicStore(uint32_t* addr, uint32_t val,
                                            ::cuda::memory_order memory_order) {
  ValidateCudaArch();
  ::cuda::atomic_ref<uint32_t, ::cuda::thread_scope_system> ref(*addr);
  ref.store(val, memory_order);
}

__device__ __forceinline__ uint32_t
AtomicLoad(uint32_t* addr, ::cuda::memory_order memory_order) {
  ValidateCudaArch();
  ::cuda::atomic_ref<uint32_t, ::cuda::thread_scope_system> ref(*addr);
  return ref.load(memory_order);
}
...
__device__ __forceinline__ void PutSignalFlag(uint32_t* addr, uint32_t val) {
  ...
  AtomicStore(addr, val, ::cuda::memory_order_release);
}
...
__device__ __forceinline__ void WaitSignalFlag(uint32_t* addr,
  ...
  while (AtomicLoad(addr, ::cuda::memory_order_acquire) != expected) {
  }
}
