...
template <typename KernelDefinitionT>
class KernelEmitter : public KernelEmitterBase {
 public:
  virtual ~KernelEmitter() = default;

  virtual absl::StatusOr<KernelDefinitionT> EmitKernelDefinition() = 0;

 private:
  absl::StatusOr<std::unique_ptr<KernelDefinitionBase>>
  EmitBaseKernelDefinition() final {
    TF_ASSIGN_OR_RETURN(KernelDefinitionT kernel_definition,
                        EmitKernelDefinition());
    return std::make_unique<KernelDefinitionT>(std::move(kernel_definition));
  }
};
