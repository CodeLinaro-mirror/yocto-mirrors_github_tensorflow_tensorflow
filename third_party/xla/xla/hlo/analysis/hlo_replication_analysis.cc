...
HloReplicationAnalysis::GroupsForReplicas(
  ...
  int64_t num_partitions = get_num_partitions();
  int64_t replica_count = get_replica_count();
  int64_t num_replicas = cross_partition_spmd_ ? replica_count : num_partitions;
    ...
    if (!hlo->channel_id().has_value()) {
      ...
      if (global_id) {
        ...
        for (const auto& device_sets : *device_sets_per_replica) {
          fully_replicated &= device_sets.size() == 1 &&
                              (*device_sets.begin()).size() ==
                                  (cross_partition_spmd_ ? get_num_partitions()
                                                         : get_replica_count());
        }
        if (fully_replicated) {
          return HloReplication::ReplicatedOnAllDevices();
