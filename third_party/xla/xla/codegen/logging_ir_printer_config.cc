...
#include "xla/codegen/logging_ir_printer_config.h"
...
bool LoggingIRPrinterConfig::ShouldDumpMlirPasses(
    const HloModule& hlo_module, absl::string_view pass_manager_name) {
  ...
  return should_dump_mlir_passes;
}

LoggingIRPrinterConfig::LoggingIRPrinterConfig(
    mlir::MLIRContext* context, const HloModule& hlo_module,
    absl::string_view kernel_name, absl::string_view pass_manager_name)
...
void LoggingIRPrinterConfig::printBeforeIfEnabled(
    mlir::Pass* pass, mlir::Operation* operation,
...
void LoggingIRPrinterConfig::printAfterIfEnabled(
    mlir::Pass* pass, mlir::Operation* operation,
    PrintCallbackFn printCallback) {
  if (log_stream_) {
    printCallback(*log_stream_);
