...
// Returns the result `expr` if it represents an LiteRT error status (either
...
// ```
// LITERT_RETURN_IF_ERROR(expr, return_expr);
// ```
//
...
#define LITERT_ASSIGN_OR_RETURN(DECL, EXPR) \
  LITERT_ASSIGN_OR_RETURN_HELPER(           \
      _CONCAT_NAME(expected_value_or_error_, __LINE__), DECL, EXPR)
...
class ErrorStatusReturnHelper {
 public:
  // NOLINTBEGIN(*-explicit-constructor): This is meant to be transparent.
  template <class T>
  ErrorStatusReturnHelper(const litert::Expected<T> error) : error_(error) {}
  ErrorStatusReturnHelper(LiteRtStatus status) : error_(status) {}
  ErrorStatusReturnHelper(litert::Unexpected error) : error_(error.Error()) {}
...
#define LITERT_RETURN_IF_ERROR_2(EXPR, RETURN_VALUE)       \
  do {                                                     \
    if (auto status = (EXPR); status != kLiteRtStatusOk) { \
      return RETURN_VALUE;                                 \
    }                                                      \
  } while (false)

#define LITERT_ASSIGN_OR_RETURN_HELPER(TMP_VAR, DECL, EXPR) \
  auto&& TMP_VAR = (EXPR);                                  \
