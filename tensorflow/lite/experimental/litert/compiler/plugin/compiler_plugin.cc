...
Expected<LiteRtParamIndex> CompiledResult::NumByteCode() const {
...
LiteRtStatus ResolvePluginApi(void* lib_handle,
  ...
  RESOLVE_API_FUNC(kLiteRtCompiledResultNumByteCode,
...
Expected<void> ApplyPlugin(CompilerPlugin& compiler_plugin, LiteRtModelT& model,
  ...
  auto num_byte_code = compiled_result->NumByteCode();
  if (!num_byte_code) {
