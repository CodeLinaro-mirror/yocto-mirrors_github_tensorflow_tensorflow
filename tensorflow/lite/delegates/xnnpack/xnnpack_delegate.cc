 ...
 public:
  static Subgraph* Create(TfLiteContext* context,
    ...
    for (int t : tensors) {
      ...
      TfLiteStatus status = DefineXNNPACKValue(context, subgraph.get(), *tensor,
                                               t, data, flags, &xnnpack_id);
      if (status != kTfLiteOk) {
        TF_LITE_KERNEL_LOG(context,
  ...
  static TfLiteStatus VisitFullyConnectedNode(
    ...
    if (subgraph != nullptr) {
      ...
      if (!fc_params->keep_num_dims) {
        ...
        int tflite_dims[3] = {2, 0, input_channels};
        TfLiteIntArray tflite_dims;
        int tflite_dims_data[2] = {0, input_channels};
        tflite_dims.size = 2;
        tflite_dims.data = tflite_dims_data;
        reshaped_tensor.dims = &tflite_dims;
        ...
        TfLiteIntArrayFree(reshaped_tensor.dims);
