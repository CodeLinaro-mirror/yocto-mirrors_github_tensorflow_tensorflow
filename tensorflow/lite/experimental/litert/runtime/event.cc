...
#if LITERT_HAS_OPENCL_SUPPORT
    cl_int res = clWaitForEvents(/*num_events=*/1,
                                 /*event_list=*/&opencl_event);
    if (res != CL_SUCCESS) {
      return Error(kLiteRtStatusErrorRuntimeFailure,
                   "Error waiting for OpenCL event");
    }
    return {};
#else
    return Error(kLiteRtStatusErrorUnsupported,
...
Expected<void> LiteRtEventT::Signal() {
  ...
  if (type == LiteRtEventTypeOpenCl) {
    cl_int res = clSetUserEventStatus(opencl_event, CL_COMPLETE);
    if (res != CL_SUCCESS) {
      return Error(kLiteRtStatusErrorRuntimeFailure,
                   "Error signaling OpenCL event");
    }
    return {};
  }
#endif
  return Error(kLiteRtStatusErrorInvalidArgument,
               "The event trigger is not supported");
}

LiteRtEventT* LiteRtEventT::CreateManaged(LiteRtEventType type) {
#if LITERT_HAS_OPENCL_SUPPORT
  if (type == LiteRtEventTypeOpenCl) {
    auto& env = litert::internal::EnvironmentSingleton::GetInstance();
    cl_int errcode_ret;
    cl_event user_event =
        clCreateUserEvent(env.getContext()->context(), &errcode_ret);
