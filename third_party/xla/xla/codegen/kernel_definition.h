...
class KernelDefinition final : public KernelDefinitionBase {
 public:
  struct Storage {
    KernelSpec spec;
    KernelSourceT source;
  };

  KernelDefinition(KernelSpec spec, KernelSourceT source)
  ...
  const KernelSourceT& source() const override { return storage_.source; }
...
template <typename KernelSourceT>
KernelDefinition(KernelSpec, KernelSourceT) -> KernelDefinition<KernelSourceT>;
