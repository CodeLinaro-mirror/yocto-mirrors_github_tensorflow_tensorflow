...
ENTRY main{
  ...
  ROOT triton_softmax = $0[127,125] fusion(p), kind=kCustom,
    ...
    {"roots":[{"output_tile_sizes":["1","125"]}],"num_warps":"1"}},"force_earliest_schedule":false}
}
  ...
  ROOT triton_softmax = f32[16,256000]{1,0} fusion(param_0), kind=kCustom,
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton","block_level_fusion_config":
        {"roots":[{"output_tile_sizes":["1","256000"]}],"num_warps":"32"}}}
}
  )",
                       "");
...
ENTRY main {
  ...
  r0 = f32[16] fusion(p0), kind=kCustom, calls=reduce_0, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["16"]}],"num_warps":"1"}}}
  r1 = f32[16] fusion(p1), kind=kCustom, calls=reduce_1, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["16"]}],"num_warps":"1"}}}
  r2 = f32[16] fusion(p2), kind=kCustom, calls=reduce_2, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["16"]}],"num_warps":"1"}}}
...
ENTRY main {
  ...
  ROOT triton_softmax = f32[1,1,16384,16384] fusion(p), kind=kCustom,
    ...
    backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],
      "fusion_backend_config":{"kind":"__triton","block_level_fusion_config":
        {"roots":[{"output_tile_sizes":["1","1","1","16384"]}],"num_warps":"32"}},
