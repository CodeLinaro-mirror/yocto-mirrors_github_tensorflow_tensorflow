...
ENTRY main {
  ...
  ROOT triton_reduction = f32[8] fusion(param_0), kind=kCustom, calls=triton_reduction_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["4"]}],"num_warps":"1"}}}
})";
...
ENTRY main {
  param_0 = f32[8,4] parameter(0)
  ROOT triton_reduction = f32[4] fusion(param_0), kind=kCustom, calls=triton_reduction_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["4"]}],"num_warps":"1"}}}
})";
  TF_EXPECT_OK(CreateTritonIrAndFileCheck(this, kHloText,
                                          "triton_reduction_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_reduction = f32[5,5,5,3] fusion(param_0), kind=kCustom, calls=triton_reduction_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["4", "2", "5", "1"]}],"num_warps":"1"}}}
})";
  TF_EXPECT_OK(CreateTritonIrAndFileCheck(this, kHloText,
                                          "triton_reduction_computation", R"(
CHECK:  tt.make_range
...
ENTRY main {
  ...
  ROOT triton_reduction = f32[3] fusion(param_0), kind=kCustom, calls=triton_reduction_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["3"]}],"num_warps":"1"}}}
})";
  TF_EXPECT_OK(CreateTritonIrAndFileCheck(this, kHloText,
                                          "triton_reduction_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_softmax = f32[125,127]{1,0} fusion(param_0),
    ...
    backend_config={"fusion_backend_config":{
      "kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["1", "128"]}],
                                   "num_warps":"1"}}}})";
  TF_EXPECT_OK(CreateTritonIrAndFileCheck(this, kHloText,
...
ENTRY main {
  ...
  ROOT triton_softmax = f32[10,125,127]{2,1,0} fusion(param_0, param_1, param_2),
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
       "block_level_fusion_config": {"roots":[{"output_tile_sizes": ["1", "1", "127"]}],
                                     "num_warps": "1"}}}
})";
...
ENTRY main {
  parameter_1 = f32[32]{0} parameter(1)
  parameter_0 = f32[32,16]{1,0} parameter(0)
  ROOT _ = f32[32,16]{1,0} fusion(parameter_0, parameter_1), kind=kCustom, calls=triton_softmax_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","16"]}],"num_warps":"1"}}}
})";
...
ENTRY main {
  p0 = pred[10,128]{1,0} parameter(0)
  ROOT softmax = f32[10,128] fusion(p0), kind=kCustom, calls=triton_softmax_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","128"]}],"num_warps":"1"}}}
})";
...
ENTRY main {
  parameter_0 = f32[16,32]{1,0} parameter(0)
  parameter_1 = f32[32]{0} parameter(1)
  ROOT _ = f32[16,32]{1,0} fusion(parameter_0,parameter_1), kind=kCustom, calls=triton_softmax_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","32"]}],"num_warps":"1"}}}
})";
...
ENTRY main {
  ...
  ROOT _ = f32[64,32,16]{2,1,0} fusion(parameter_1, parameter_0), kind=kCustom,
    ...
    backend_config={"fusion_backend_config":{
      "kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","1","16"]}],
                                   "num_warps":"1"}}}
}
)";
...
ENTRY main {
  ...
  ROOT _ = f32[64,32,16]{2,1,0} fusion(f32[64,32,16]{2,1,0} parameter_1, f32[16]{0} parameter_0), kind=kCustom, calls=%triton_softmax_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","1","16"]}],"num_warps":"1"}}}
}
)";
...
ENTRY entry {
  ...
  ROOT r = f32[10,10] fusion(p0, p1),
    ...
    backend_config={"fusion_backend_config": {
      "kind":"__triton",
      "block_level_fusion_config": {"roots":[{"output_tile_sizes": ["1","1"]}],
                                    "num_warps": "1"}}}
})";
...
ENTRY entry_computation {
  ...
  ROOT fusion = f32[8192,50304] fusion(param_0),
    ...
    backend_config={"fusion_backend_config": {
      "kind":"__triton",
      "block_level_fusion_config": {"roots":[{"output_tile_sizes": ["1024","1"]}],
                                    "num_warps": "1"}}}
})"));
  const HloFusionInstruction* triton_fusion = Cast<HloFusionInstruction>(
      hlo_module->entry_computation()->root_instruction());
...
ENTRY main {
  ...
  ROOT triton_reduction = f32[125]{0} fusion(param_0, param_1),
    ...
    backend_config={"fusion_backend_config": {
      "kind":"__triton",
      "block_level_fusion_config": {"roots":[{"output_tile_sizes": ["1"]}],
                                    "num_warps": "1"}}}})";
  TF_EXPECT_OK(CreateTritonIrAndFileCheck(this, kHloText,
                                          "triton_reduction_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_reduce = f32[4,12,125]{2,1,0} fusion(param_0),
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["2","5","16"]}],"num_warps":"4"}}}
})";
...
ENTRY entry_computation {
  ...
  ROOT fusion = f32[4,4,8] fusion(param_0.2), kind=kCustom, calls=triton_softmax_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["2","2","8"]}],"num_warps":"1"}}}
})";
...
ENTRY entry_computation {
  ...
  backend_config={"fusion_backend_config":
    {"kind":"__triton","block_level_fusion_config":
      {"roots":[{"output_tile_sizes":["8","4"]}],"num_warps":"1"}}}
})";
...
ENTRY entry_computation {
  ...
  ROOT fusion = f32[4,4,8] fusion(param_0.2), kind=kCustom, calls=fused_computation, backend_config={"fusion_backend_config": {"kind":"__triton","block_level_fusion_config":{"roots":[{"output_tile_sizes":["2","2","8"]}],"num_warps":"1"}}}
})";
  EXPECT_TRUE(RunAndCompareNoHloPasses(kHloText, ErrorSpec{/*aabs=*/1e-6,
                                                           /*arel=*/1e-6}));
}
  ...
  ROOT fusion = f32[3,2,14] fusion(p),
    kind=kCustom, calls=fused_computation, backend_config={
      "fusion_backend_config": {
        "kind":"__triton","block_level_fusion_config": {
          "roots":[{"output_tile_sizes":["2","2","8"]}],"num_warps":"1"
        }
      }
    }
})";
...
ENTRY main {
  ...
  ROOT triton_fusion = f32[64,2,256,2]{3,2,1,0} fusion(param_0), kind=kCustom,
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["2","2","2","2"]}],
                                   "num_warps":"1"}}}
})";
  TF_EXPECT_OK(
...
ENTRY main {
  ...
  ROOT triton_fusion = f32[64,2,256,2]{3,2,1,0} fusion(param_0), kind=kCustom,
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["4","2","8","2"]}],
                                   "num_warps":"1"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
  ...
  ROOT fusion = s8[5,6,7] fusion(p), kind=kCustom, calls=triton_computation,
    backend_config={
      "fusion_backend_config":{ "kind":"__triton", "block_level_fusion_config":{
          "roots":[{"output_tile_sizes":["2","4","1"]}], "num_warps":"1"}}
    }
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY entry_computation {
  ...
  ROOT fusion = s8[5,42] fusion(p), kind=kCustom, calls=triton_computation,
    backend_config={
      "fusion_backend_config":{ "kind":"__triton", "block_level_fusion_config":{
          "roots":[{"output_tile_sizes":["4","1"]}], "num_warps":"1"}}
    }
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY main {
  ...
  backend_config={"fusion_backend_config":
    {"kind":"__triton",
     "block_level_fusion_config":{"roots":[{"output_tile_sizes":["8","4","128"]}],
                                  "num_warps":"8"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_fusion = f32[127,125]{1,0} fusion(param_0), kind=kCustom,
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["8","4"]}],
                                   "num_warps":"1"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_fusion = pred[15] fusion(param_0, param_1), kind=kCustom,
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["4"]}],
                                   "num_warps":"1"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_fusion = f32[3,15,7] fusion(param_0),
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","8","4"]}],
                                   "num_warps":"1"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY main {
  ...
  ROOT triton_fusion = f32[8,3,20] fusion(param_0),
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","1", "20"]}],
                                   "num_warps":"4"}}}
})";
  EXPECT_TRUE(
      RunAndCompareNoHloPasses(kHloText, ErrorSpec{/*aabs=*/0, /*arel=*/0}));
...
ENTRY main {
  ROOT triton_fusion = f32[3,4,182,5] fusion(),
    ...
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"roots":[{"output_tile_sizes":["1","2","64","8"]}],
                                   "num_warps":"1"}}}
})";
...
ENTRY entry_computation {
  ...
  ROOT fusion = f32[5,7] fusion(p), kind=kCustom, calls=triton_computation,
    backend_config={
      "fusion_backend_config":{ "kind":"__triton", "block_level_fusion_config":{
          "roots": [{"output_tile_sizes":["4","4"]}], "num_warps":"1"}}
    }
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY entry_computation {
  ...
  ROOT fusion = bf16[] fusion(p0, p1), kind=kCustom, calls=triton_computation,
    backend_config={
      "fusion_backend_config":{ "kind":"__triton", "block_level_fusion_config":{
          "roots": [{"output_tile_sizes":[]}], "num_warps":"1"}}
    }
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
...
ENTRY entry_computation {
  ...
  ROOT r = s32[2] fusion(p),
     ...
     backend_config={
     "fusion_backend_config":{"kind":"__triton","block_level_fusion_config":
     {"roots":[{"output_tile_sizes":["2"]}],"num_warps":"1"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
CHECK:     tt.load
    ...
    backend_config={
      "fusion_backend_config":{
        ...
        "block_level_fusion_config":{
          "roots":[{"output_tile_sizes":["2","256"]}],"num_warps":"1"}}}
