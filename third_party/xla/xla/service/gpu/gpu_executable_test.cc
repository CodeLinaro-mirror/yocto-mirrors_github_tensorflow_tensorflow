...
TEST(GpuExecutableTest, RunThunkPasses) {
  ...
  auto create_executable = [&]() {
    ...
    params.gpu_device_info = device_description;
  ...
  // Create multiple executables to check for unique dump files.
  ASSERT_OK(create_executable());
  ASSERT_OK(create_executable());
  auto executable = create_executable();
  ASSERT_TRUE(executable.ok());

  const SequentialThunk& thunk = (*executable)->GetThunk();
  const ThunkSequence& thunks = thunk.thunks();
  ASSERT_EQ(thunks.size(), 1);
  EXPECT_EQ(thunks[0]->kind(), Thunk::kCommandBuffer);

  // Check for unique dump files.
  std::vector<std::string> dump_files;
  ASSERT_TRUE(tsl::Env::Default()
                  ->GetMatchingPaths(
                      tsl::io::JoinPath(
                          dump_dir, "*thunk_sequence_after_thunk_passes_*.txt"),
                      &dump_files)
                  .ok());
