...
TEST_F(GpuProfilerTest, ProfileWithSharedBuffers) {
  ...
  GpuProfiler profiler(stream_exec_, ProfileOptions());
  TF_ASSERT_OK_AND_ASSIGN(
      auto profiles, profiler.ProfileWithSharedBuffers(std::move(executables)));
  EXPECT_EQ(profiles.size(), 2);
  EXPECT_EQ(profiles[0].duration, absl::Nanoseconds(1000));
  EXPECT_EQ(profiles[1].duration, absl::Nanoseconds(2000));
}
