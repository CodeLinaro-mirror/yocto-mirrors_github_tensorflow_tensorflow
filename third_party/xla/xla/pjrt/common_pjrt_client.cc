...
CommonPjRtClient::BufferFromHostLiteral(const LiteralSlice& literal,
  ...
  return DefineBuffer(std::make_shared<const Shape>(std::move(device_shape)),
                      memory_space, std::move(raw_buffer),
...
CommonPjRtClient::CreateUninitializedBuffer(const Shape& shape,
  ...
  TF_ASSIGN_OR_RETURN(
      auto output_buffer,
      DefineBuffer(std::make_shared<const Shape>(std::move(device_shape)),
                   memory_space, raw_buffer, {std::move(definition_event)}));
  ...
  if (host_buffer_semantics ==
    ...
    if (BufferFromHostBufferSupportsZeroCopy(data, type, dims, byte_strides,
      ...
      TF_ASSIGN_OR_RETURN(
          auto output_buffer,
          DefineBuffer(
              shared_device_shape, memory_space, raw_buffer,
...
CommonPjRtClient::CreateViewOfDeviceBuffer(
  ...
  TF_ASSIGN_OR_RETURN(
      auto output_buffer,
      DefineBuffer(
          std::make_shared<const Shape>(std::move(device_shape)), memory_space,
          raw_buffer,
          absl::InlinedVector<tsl::RCReference<PjRtDeviceEvent>, 4>{}));
  return output_buffer;
}
