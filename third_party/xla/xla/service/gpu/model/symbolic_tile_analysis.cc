...
// It is assumed that `specification_for_instruction` does not contain any
// information regarding the nested tiling parameters of the fusion.
//
// `specification_for_instruction` is however allowed to contain information
...
absl::Status PopulateNestedParameters(
  ...
  auto set_specification_for_instruction =
      [&](const HloInstruction& instruction, int64_t num_parameters) {
        ...
        auto it = specification_for_instruction.find(&instruction);

        // If the instruction is already in the specification, update it. This
        // should in principle only occur if the instruction has both parameters
        // visible at the output as well as hidden parameters. A `dot` that
        // is the root of a fusion will model this case, for example.
        if (it != specification_for_instruction.end()) {
          specification_for_instruction[&instruction] =
              it->second + num_parameters;
        } else {
          specification_for_instruction[&instruction] = num_parameters;
        }
  ...
  for (auto& instruction_adaptor : fusion.MakeInstructionPostOrder()) {
    ...
    if (instruction_adaptor.opcode() == HloOpcode::kFusion) {
      ...
      TF_RETURN_IF_ERROR(PopulateNestedParameters(
          *nested_fusion_adaptor, specification_for_instruction));
      continue;
    }
    ...
    if (instruction_adaptor.opcode() == HloOpcode::kDot) {
      int64_t num_parameters = instruction_adaptor.instruction()
                                   ...
                                   .size();
      TF_RETURN_IF_ERROR(set_specification_for_instruction(
...
/*static*/ absl::StatusOr<TilingSpecification> TilingSpecification::FromFusion(
  ...
  TF_RETURN_IF_ERROR(
      PopulateNestedParameters(fusion_adaptor, specification_for_instruction));
  return TilingSpecification(std::move(specification_for_instruction));
}
...
/*static*/ absl::StatusOr<TilingSpecification> TilingSpecification::FromFusion(
  ...
  return TilingSpecification::FromFusion(*fusion_adaptor);
}
...
bool Tiling::ConformsTo(const TilingSpecification& tiling_specification) const {
  const absl::flat_hash_map<const HloInstruction*, int64_t>&
      specification_by_instruction =
          tiling_specification.specification_by_instruction();
  if (tile_sizes_.size() != specification_by_instruction.size()) {
    return false;
  }
  for (const auto& [hlo, num_parameters] : specification_by_instruction) {
