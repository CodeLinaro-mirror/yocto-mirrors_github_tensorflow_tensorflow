...
const char key_prefix[] = "ifrt_cross_host_transfer_";
...
absl::StatusOr<PjRtArray::PjRtBuffers> PjRtClient::CrossHostReceiveBuffers(
  ...
  auto notifier = [this, keys](
    ...
    if (!recv_state.ok()) {
      LOG(ERROR) << "Invalid PjRtCrossHostRecvState passed to "
                    "xla::PjRtCrossHostRecvNotifier callback in "
                    "xla::PjRtClient::MakeCrossHostReceiveBuffers: "
                 << recv_state.status();
    }
    CHECK_OK(recv_state);
    auto on_canceled = [](absl::Status status) {
      if (!status.ok()) {
        LOG(ERROR) << "Invalid status passed to `on_canceled` callback in "
