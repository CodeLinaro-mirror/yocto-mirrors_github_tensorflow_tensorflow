...
class DutyCycleTracker {
 public:
  ...
  TimePs GetActiveTime() const;
  TimePs GetIdleTime() const;
  TimePs GetDuration() const { return total_time_span_.duration_ps(); }
  double DutyCycle() const {
    return tsl::profiler::SafeDivide(GetActiveTime(), GetDuration());
  }

 private:
  struct TimespanComparator {
    // Order by begin_ps increasing then duration decreasing.
  ...
  void MergeOverlappingTimespans(ActiveTimeSpans::const_iterator begin,
