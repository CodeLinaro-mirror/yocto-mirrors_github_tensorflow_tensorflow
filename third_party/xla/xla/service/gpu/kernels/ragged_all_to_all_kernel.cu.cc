...
template <typename T>
__global__ void RaggedAllToAllKernel(
    T* input_ptr,
    std::array<void*, kMaxNumRaggedAllToAllOutputPtrs> output_ptrs,
    int64_t* input_offsets_ptr, int64_t* send_sizes_ptr,
    int64_t* output_offsets_ptr, int64_t num_updates_per_replica,
    int64_t num_row_elements) {
  int64_t update_idx = blockIdx.x;
  int64_t output_idx = update_idx / num_updates_per_replica;

  T* output_ptr = reinterpret_cast<T*>(output_ptrs[output_idx]);
