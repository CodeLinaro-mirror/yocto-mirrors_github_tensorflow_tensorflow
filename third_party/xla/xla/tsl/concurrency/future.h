...
class FutureBase : public FutureMoveControl<is_move_only> {
 ...
 protected:
  ...
  // to the future will be executed by in a thread, that sets the promise value.
  // This might lead to running expensive callbacks on a thread that is not
  // designed for that, i.e. if a promise is set by a non-blocking thread that
  // handles IO events, running expensive computation might lead to performance
  // issues in the system.
  //
  // Detached future guarantees that all callbacks will be executed on the
  // specified executor. Users can override executor by using `OnReady` and
  // `Map` overloads, that accept another executor instance.
  //
  // We use a trick we an extra template parameter to disable this overload when
  // T is move-only, as we don't want to allow to create multiple futures
...
class Future : public internal::FutureBase<absl::StatusOr<T>> {
 ...
 public:
  ...
  // promise will fulfill the connected future and will run all callbacks in the
  // caller thread.
  //
  ...
  // Returns a pair of connected Promise and Future<>. Setting the returned
  // promise will fulfill the connected future and will run all callbacks on the
  // given `executor`.
  //
  // - on_block_start is called before Await starts to block.
  // - on_block_end is called after Await finishes blocking.
