...
static ynn_status define_batch_matrix_multiply(
    ynn_subgraph_t subgraph, uint32_t input1_id, uint32_t input2_id,
    uint32_t output_id, size_t b_rank, bool transpose_b) {
  if (transpose_b) {
...
static absl::StatusOr<YnnSubgraph> EmitYnnDotSubgraph(
  ...
  YNN_RETURN_IF_ERROR(define_batch_matrix_multiply(
      subgraph.get(), lhs_id, rhs_id, out_id, b_rank, transpose_b));
