...
absl::StatusOr<absl::flat_hash_set<HloInstruction*>> GetSyncCollectives(
  ...
  stream_executor::DeviceDescription device_info;
  device_info.set_device_memory_size(20000);
  return xla::gpu::GetSyncCollectives(module, pointer_size, device_info);
}

TEST_F(CollectiveCombinerUtilsTest, GetSyncCollectivesNoOverlap) {
  absl::string_view kHloText = R"(
    HloModule m
  ...
  EXPECT_THAT(GetSyncCollectives(*module),
              IsOkAndHolds(UnorderedElementsAre(ar0, ar1)));
}

TEST_F(CollectiveCombinerUtilsTest, GetSyncCollectivesWithOverlap) {
  // Expected schedule:
  // ------------------
  // c0 –> ar0
  //       c1 –> ar1
