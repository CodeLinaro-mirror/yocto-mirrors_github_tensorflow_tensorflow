...
absl::StatusOr<std::unique_ptr<FunctionLibrary>> ObjectLoader::Load(
  ...
  // TODO(kbasioli) do we need to mangle? If so maybe we need the target
  // machine -> i would expect that we should be able to add metadata to the
  // proto to indicate the mangling.
  ...
  for (const auto& symbol : symbols) {
    ...
    // TODO(basioli) mangling is discussed in the comment above.
    lookup_set.add(execution_session_->intern(symbol.name));
  }
  ...
  for (const auto& symbol : symbols) {
    ...
    // TODO (kbasioli) mangling is discussed in the comment above.
    auto symbol_name = execution_session_->intern(symbol.name);
