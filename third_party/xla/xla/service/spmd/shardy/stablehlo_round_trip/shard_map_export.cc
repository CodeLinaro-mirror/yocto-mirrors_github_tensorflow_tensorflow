  ...
  op.getBody().front().walk<mlir::WalkOrder::PreOrder>(
      [&](Operation* opInBody) {
        ...
        if (mesh.getAxes().size() == manualAxes.region.size()) {
          ...
          hloShardingAttr = convertToHloShardingAttr(
              opInBody, shardingPerValue.getShardings(), getMeshAttr,
              manualAxes.region);
        }

        opInBody->setAttr(kXlaShardingAttr, hloShardingAttr);
