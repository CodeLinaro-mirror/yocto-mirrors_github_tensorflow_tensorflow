...
std::unique_ptr<HloComputation> CreateAdd(PrimitiveType type) {
...
std::unique_ptr<HloInstruction> CreateCumulativeSum(
    HloModule* module, HloInstruction* group_sizes) {
  int batch_dims = group_sizes->shape().dimensions().size() - 1;
  int num_groups = group_sizes->shape().dimensions(batch_dims);
  ...
  HloComputation* add = module->AddEmbeddedComputation(CreateAdd(type));
...
// that group.
std::unique_ptr<HloInstruction> RaggedToDense(HloModule* module,
                                              HloInstruction* ragged_op,
                                              HloInstruction* group_sizes,
                                              int new_dim_index,
                                              int ragged_dim) {
  auto computation = ragged_op->parent();
  HloInstruction* cumulative_sum =
      computation->AddInstruction(CreateCumulativeSum(module, group_sizes));
  ...
  auto zero = computation->AddInstruction(Zero(group_type));
  auto zero_arr = computation->AddInstruction(
      HloInstruction::CreateBroadcast(zero_slice_shape, zero, {}));
  auto concat = computation->AddInstruction(HloInstruction::CreateConcatenate(
      group_sizes->shape(), {zero_arr, slice}, group_dim));

  // Broadcast cumulative sum array to LHS shape + group dimension.
  auto old_shape = ragged_op->shape();
  ...
  auto broadcast_ragged =
      computation->AddInstruction(HloInstruction::CreateBroadcast(
          new_shape, ragged_op, old_dims_new_shape));

  zero = computation->AddInstruction(Zero(element_type));
...
absl::StatusOr<std::unique_ptr<HloInstruction>> RaggedToGeneral(
    HloModule* module, HloRaggedDotInstruction* ragged_dot) {
  ...
  switch (mode) {
    case RaggedDotMode::kRaggedNonContracting: {
      ...
      lhs = computation->AddInstruction(RaggedToDense(
          module, lhs, group_sizes, new_dim_index, lhs_ragged_dim));
    ...
    case RaggedDotMode::kRaggedContracting: {
      ...
      rhs = computation->AddInstruction(RaggedToDense(
          module, rhs, group_sizes, new_dim_index, rhs_ragged_dim));
...
absl::StatusOr<bool> RaggedDotRewriter::Run(
  ...
  for (auto* ragged_dot : ragged_dots) {
    TF_ASSIGN_OR_RETURN(auto general_dot, RaggedToGeneral(module, ragged_dot));
