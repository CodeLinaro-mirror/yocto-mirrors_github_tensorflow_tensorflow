// RUN: xla-opt %s \
// RUN:   --triton-xla-contract-dims="finalize=false contract-store=true" \
// RUN: | FileCheck %s
...
tt.func @fold_contract_of_expand_swapping(%arg0: tensor<4x1x8xf32>) -> tensor<1x4x8xf32> {
  // CHECK: triton_xla.contract_dims {{.*}} {axis = 1 : i32} : tensor<4x1x8xf32> -> tensor<4x8xf32>
...
// CHECK-LABEL: func @contract_reshape
tt.func @contract_reshape(%arg0: tensor<4x1x1xf32>) -> tensor<4xf32> {
  // CHECK: triton_xla.contract_dims {{.*}} {axis = 1 : i32} : tensor<4x1x1xf32> -> tensor<4x1xf32>
  // CHECK: triton_xla.contract_dims {{.*}} {axis = 1 : i32} : tensor<4x1xf32> -> tensor<4xf32>
...
tt.func @reshape_with_encoding(%arg0: tensor<1x32xf32, #arg_enc>) -> tensor<32xf32, #res_enc> {
  // CHECK: triton_xla.contract_dims {{.*}} {axis = 0 : i32} :
  // CHECK-SAME: tensor<1x32xf32, #[[ARG_ENC]]> -> tensor<32xf32, #[[RES_ENC]]>
...
tt.func @contract_store(%arg0: !tt.ptr<f32>, %arg1: tensor<4x1x8xf32>) {
  ...
  %c4_i64 = arith.constant 4 : i64
  %c8_i64 = arith.constant 8 : i64
  // CHECK-DAG: triton_xla.contract_dims {{.*}} {axis = 1 : i32} : tensor<4x1x8xf32> -> tensor<4x8xf32>
...
// Not really a test, but rather an example where we mayduplicate code when we
// can't push contract_dims up through both branches. This only happens for pure
...
tt.func @diamond_may_duplicate_code(%arg0: tensor<4x1xf32>) -> tensor<4xf32> {
  ...
  %5 = triton_xla.contract_dims %4 {axis = 1 : i32} : tensor<4x1xf32> -> tensor<4xf32>
  tt.return %5 : tensor<4xf32>
}

// FINALIZE-LABEL: func @contract_dims_to_reshape
