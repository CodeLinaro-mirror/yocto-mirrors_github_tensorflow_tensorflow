...
TEST(CommandBufferConversionPassTest, ConvertsToCommandBufferThunk) {
  ...
  hlo_config.mutable_debug_options().clear_xla_gpu_enable_command_buffer();
  hlo_config.mutable_debug_options().add_xla_gpu_enable_command_buffer(
      DebugOptions::FUSION);
...
TEST(CommandBufferConversionPassTest, PartiallyConvertsToCommandBufferThunk) {
  ...
  // Enable only Fusion, which means CopyThunk should be converted.
  hlo_config.mutable_debug_options().clear_xla_gpu_enable_command_buffer();
  hlo_config.mutable_debug_options().add_xla_gpu_enable_command_buffer(
      DebugOptions::FUSION);
  se::DeviceDescription device_info;
  ...
  // Check the first command buffer thunk
  ...
  // Check the second command buffer thunk
  auto* command_buffer_thunk1 =
      static_cast<const CommandBufferThunk*>(root_thunk->thunks()[2].get());
