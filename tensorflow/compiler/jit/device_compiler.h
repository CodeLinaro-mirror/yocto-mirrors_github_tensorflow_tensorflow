...
inline NameAttrList GetDeviceCompilerFunction(
    std::string name, proto2::Map<std::string, AttrValue> attrs) {
  NameAttrList function;
  function.set_name(std::move(name));
  *function.mutable_attr() = std::move(attrs);
...
inline NameAttrList GetDeviceCompilerFunction(const NodeDef& def) {
  return GetDeviceCompilerFunction(def.op(), def.attr());
}
